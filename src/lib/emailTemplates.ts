...
export function applicantIneligibleNotification(params: ApplicantIneligibleParams): TemplateResult {
  ...
  const futureText = t(
    "If you move to Canada or change your relocation plans, contact us at irefair@andbeyondca.com and we'll update your profile.",
    "Si vous déménagez au Canada ou changez vos plans de relocalisation, contactez-nous à irefair@andbeyondca.com et nous mettrons à jour votre profil.",
    locale
  );
...
type ReferrerMobileLoginLinkParams = {
  name: string;
  iRref: string;
  appLoginUrl: string;
...
export function referrerMobileLoginLink(params: ReferrerMobileLoginLinkParams): TemplateResult {
  const {
    name,
    iRref,
    appLoginUrl,
  ...
  const subject = t(
    'Sign in to iRefair app - Referrer portal',
    'Connexion à l’app iRefair - Portail référent',
    locale,
  );

  const greeting = `${t('Hi', 'Bonjour', locale)} ${escapeHtml(name)},`;
  const eyebrowText = t('APP SIGN-IN LINK', 'LIEN DE CONNEXION APP', locale);
  const iRrefLabel = t('iRREF', 'iRREF', locale);

  const mainText = t(
    'Tap the button below from your iPhone to securely sign in to your referrer portal in the iRefair app.',
    'Touchez le bouton ci-dessous depuis votre iPhone pour vous connecter en toute sécurité à votre portail référent dans l’application iRefair.',
    locale,
  );
  const securityNote = t(
    'This sign-in link is one-time use and expires in 15 minutes.',
    'Ce lien de connexion est à usage unique et expire dans 15 minutes.',
    locale,
  );
  const ctaButton = t('Open iRefair app', 'Ouvrir l’application iRefair', locale);
  const fallbackText = t(
    "If the button doesn't open the app, copy and paste this link in Safari on your iPhone:",
    "Si le bouton n’ouvre pas l’application, copiez et collez ce lien dans Safari sur votre iPhone :",
    locale,
  );
  const portalFallbackText = t(
    'If needed, you can still access the web portal:',
    'Au besoin, vous pouvez toujours accéder au portail web :',
    locale,
  );
  const webPortalLabel = t('Web portal', 'Portail web', locale);

  const normalizedFallbackPortalUrl = fallbackPortalUrl ? normalizeHttpUrl(fallbackPortalUrl) : null;
  ...
  const content = `
    ...
    <div style="text-align:center;margin:24px 0;">
      ${button(ctaButton, appLoginUrl, 'primary')}
    </div>
    <div style="margin:20px 0;padding:16px;border-radius:12px;background:#f8fafc;border:1px solid #e6e9f0;">
      <p style="margin:0 0 8px 0;font-size:13px;color:#5c6675;">${escapeHtml(fallbackText)}</p>
      <p style="margin:0;font-size:12px;font-family:monospace;color:#1f2a37;word-break:break-all;background:#ffffff;padding:8px;border-radius:6px;">${escapeHtml(appLoginUrl)}</p>
    </div>
    <p style="margin:16px 0 0 0;font-size:13px;line-height:1.6;color:#68707f;">
      ${escapeHtml(securityNote)}
    </p>
    ${normalizedFallbackPortalUrl ? `${divider}
      <p style="margin:16px 0 8px 0;font-size:14px;line-height:1.7;color:#3b4251;">
        ${escapeHtml(portalFallbackText)}
      </p>
      <p style="margin:0 0 2px 0;font-size:14px;color:#1f2a37;">
        <strong>${escapeHtml(webPortalLabel)}:</strong> <a href="${escapeHtml(normalizedFallbackPortalUrl)}" target="_blank" rel="noreferrer" style="color:${colors.primary};text-decoration:underline;">${escapeHtml(normalizedFallbackPortalUrl)}</a>
      </p>` : ''}
  `;

  const preheader = t(
    'Use this secure one-time link to sign in to iRefair app.',
    'Utilisez ce lien sécurisé à usage unique pour vous connecter à l’application iRefair.',
    locale,
  );
...
${ctaButton}: ${appLoginUrl}

${securityNote}
${normalizedFallbackPortalUrl ? `\n${portalFallbackText}\n${webPortalLabel}: ${normalizedFallbackPortalUrl}\n` : ''}
- ${t('The iRefair team', 'L\'équipe iRefair', locale)}`;
