...
import { REFERRER_SHEET_NAME } from '../src/lib/sheets';
...
async function deduplicateReferrers() {
  ...
  const referrers: ReferrerRecord[] = [];
  ...
  // Group by email
  const emailGroups = new Map<string, ReferrerRecord[]>();
  ...
  // Find duplicates
  const duplicates: { email: string; records: ReferrerRecord[] }[] = [];
